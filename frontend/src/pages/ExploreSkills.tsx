...
const ExploreSkills = () => {
  ...
  const [matchedTeachers, setMatchedTeachers] = useState<number[]>([]);
  ...
  const handleCreateMatch = async (receiverId: number, skillName: string) => {  
    try {
      ...
      setMatchedTeachers(prev => [...prev, receiverId]);
    } catch (err: any) {
          ...
          <div className="bg-white rounded-2xl shadow p-4">
            {matches.length === 0 ? (
              ...
              matches.map((match, idx) => (
                <div
                  key={idx}
                  className="mb-3 border-b pb-3 border-gray-200"
                >
                  <h6 className="fw-bold text-primary">{match.skillName}</h6>
                  {match.teachers.length > 0 ? (
                      ...
                      {match.teachers.map((teacher) => (
                        ...
                        >
                          ...
                          <button
                            ...
                            disabled={matchedTeachers.includes(teacher.id)}
                            onClick={() => handleCreateMatch(teacher.id, match.skillName)}
                          >
                            {matchedTeachers.includes(teacher.id) ? "Match feito" : "Fazer Match"}
                          </button>
                        </li>
                      ))}
                    </ul>
                  ) : (
                    <p className="text-muted">Nenhum usuário ensinando ainda.</p>
                  )}
                </div>
              ))
            )}
